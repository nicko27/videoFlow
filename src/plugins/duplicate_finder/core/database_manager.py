...
class VideoDatabase:
    ...
    def has_video_hash(self, file_path: str) -> bool:
        """
        Check if video hash exists in database.
        ...
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT 1 FROM video_files WHERE file_path = ? AND hash_data IS NOT NULL
                ''', (file_path,))
    ...
    def get_files_without_hash(self, file_paths: list) -> list:
        """
        Get list of files that don't have hashes in database.

        Args:
            file_paths: List of file paths to check

        Returns:
            list: List of file paths without hashes
        """
        missing = []
        for path in file_paths:
            if not self.has_video_hash(path):
                missing.append(path)
        return missing
