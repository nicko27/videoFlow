...
from PyQt6.QtWidgets import (
    ...
    QSplitter, QApplication, QLabel, QTabWidget, QProgressDialog
)
from PyQt6.QtCore import Qt, pyqtSignal, QTimer
from PyQt6.QtGui import QFont, QShortcut, QKeySequence

# Import local modules (relative imports should always work in plugin context)
...
class DuplicateFinderWindow(QMainWindow):
    ...
    def _create_left_panel(self):
        ...
        callbacks = {
            ...
            'analyze_duplicateflow': self.start_duplicateflow_analysis,  # NEW
        ...
        for child in panel.findChildren(QWidget):
            if hasattr(child, 'analyze_btn'):
                ...
                if hasattr(child, 'duplicateflow_btn'):
                    self.duplicateflow_btn = child.duplicateflow_btn
                break
    ...
    def start_duplicateflow_analysis(self) -> None:
        """
        Start analysis using duplicateFlow (advanced multi-algorithm comparison).

        This method uses the DuplicateFlowWorker with 19 algorithms organized
        in 6 preset pipelines for more accurate duplicate detection.
        """
        # Check file count
        file_count = self.file_handler.get_file_count()
        ...
        if file_count < 2:
            QMessageBox.warning(
                self, "Attention",
                "Au moins 2 fichiers sont requis pour l'analyse duplicateFlow"
            )
            return
        ...
        # IMPORTANT: Force hash computation for files not in database
        missing_hashes = self.db_manager.get_files_without_hash(valid_files)
        if missing_hashes:
            logger.info(f"🔢 Computing hashes for {len(missing_hashes)} files not in database...")

            # Show progress dialog
            progress = QProgressDialog(
                f"Calcul des hashs MD5 pour {len(missing_hashes)} fichiers...",
                "Annuler", 0, len(missing_hashes), self
            )
            progress.setWindowTitle("Préparation de l'analyse")
            progress.setWindowModality(Qt.WindowModality.WindowModal)
            progress.setMinimumDuration(0)

            # Compute hashes
            for i, file_path in enumerate(missing_hashes):
                if progress.wasCanceled():
                    logger.info("Hash computation canceled by user")
                    return

                progress.setLabelText(f"Calcul du hash MD5: {Path(file_path).name}")
                progress.setValue(i)

                try:
                    # Use VideoHasher to compute and store hash
                    from ..processing.video_hasher import VideoHasher
            ...
            progress.close()
            logger.info(f"✅ All hashes computed successfully")

        # Show info dialog about duplicateFlow
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Icon.Information)
        msg.setWindowTitle("Analyse duplicateFlow")
        msg.setText(
            f"🔬 Analyse avancée duplicateFlow\n\n"
            f"Fichiers à analyser: {len(valid_files)}\n"
            f"Comparaisons: {len(valid_files) * (len(valid_files) - 1) // 2}\n\n"
            f"Algorithmes: 19 (vs 5 legacy)\n"
            f"Preset: Balanced (~2min par heure de vidéo)\n"
            f"Multi-algorithm voting pour meilleure précision"
        )
        ...
        logger.info(f"🔬 Starting duplicateFlow analysis: {len(valid_files)} files")

        # Get threshold
        threshold = 70.0  # Default
        if hasattr(self, 'threshold_spin') and self.threshold_spin:
        ...
        # Update UI
        if hasattr(self, 'duplicateflow_btn') and self.duplicateflow_btn:
            self.duplicateflow_btn.setEnabled(False)
        if self.analyze_btn:
            self.analyze_btn.setEnabled(False)
        if self.stop_btn:
        ...
        # Update status
        self.status_indicator.update_status(
            "🔬", "duplicateFlow analysis running...",
            "#9C27B0", "#F3E5F5", "#9C27B0"
        )

        # Start UI updates
        self.start_ui_updates()

        # Start duplicateFlow comparison
        try:
            self.analysis_handler.start_duplicateflow_comparison(
                files=valid_files,
                preset='balanced',  # TODO: Make this configurable via UI
                threshold=threshold,
                duplicate_callback=self._on_duplicate_found,
                progress_callback=self._on_comparison_progress,
                status_callback=self._on_status_update,
                comparison_details_callback=self._on_comparison_details
            )

            logger.info("duplicateFlow worker started successfully")

        except Exception as e:
            logger.error(f"Failed to start duplicateFlow analysis: {e}", exc_info=True)
            QMessageBox.critical(
                self, "Erreur",
                f"Échec du démarrage de duplicateFlow:\n{str(e)}"
            )

            # Reset UI on error
            self.set_analysis_mode(False)
            if hasattr(self, 'duplicateflow_btn') and self.duplicateflow_btn:
                self.duplicateflow_btn.setEnabled(True)
            if self.analyze_btn:
                self.analyze_btn.setEnabled(True)
            if self.stop_btn:
                self.stop_btn.setEnabled(False)
