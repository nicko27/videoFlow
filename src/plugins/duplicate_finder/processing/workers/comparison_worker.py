...
class OptimizedComparisonWorker(QThread):
    ...
    def __init__(
        ...
        # CORRECTION BUG #6: Use threshold normalization utility
        from ...utils.threshold_utils import normalize_threshold
        self.threshold = normalize_threshold(threshold or 0.85)
    ...
    def generate_pairs(self, files: List[str], specific_pairs: Optional[List[tuple]] = None) -> List[Tuple[str, str]]:
        ...
        **AUDIO-FIRST MODE**: If specific_pairs provided, only filter those pairs.
        ...
        try:
            ...
            logger.debug(f"Préchargement des paires ignorées impossible, on continue pair par pair: {preload_err}")
